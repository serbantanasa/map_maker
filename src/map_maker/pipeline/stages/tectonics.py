...
def _cube_net_rgb(faces: np.ndarray) -> np.ndarray:
    ...
    net = np.zeros((resolution * 3, resolution * 4, 3), dtype=faces.dtype)
...
def _tectonics_visualizer(
    ...
    if plate_data is None or plate_data.ndim not in (3, 4):
        return None
    minimum_components = (
        SPHERICAL_PLATE_FIELD_COMPONENTS if plate_data.ndim == 4 else PLATE_FIELD_COMPONENTS
    )
    if plate_data.shape[-1] < minimum_components:
        return None

    spatial_shape = plate_data.shape[:-1]
    ...
    classified = np.zeros((*spatial_shape, 3), dtype=np.uint8)
    for pid in unique_ids:
        mask = plate_ids == pid
        if not np.any(mask):
            continue
        base = palette[int(pid) % len(palette)]
        color = np.clip(base + rng.uniform(-18.0, 18.0, size=3), 0.0, 255.0).astype(np.uint8)
    ...
    Image.fromarray(_cube_net_rgb(classified), mode="RGB").save(plates_path)
    results.append(VisualizationResult(path=plates_path, artifact_name="PlateField", metadata={}))

    crust = np.zeros((*spatial_shape, 3), dtype=np.uint8)
    crust[continental_mask] = np.array([183, 157, 104], dtype=np.uint8)
    crust[~continental_mask] = np.array([41, 101, 151], dtype=np.uint8)
    crust_path = request.output_dir / "crust_provinces.png"
    Image.fromarray(_cube_net_rgb(crust), mode="RGB").save(crust_path)
    results.append(
        VisualizationResult(path=crust_path, artifact_name="ContinentalCrust", metadata={})
    )
    ...
    if convergence is not None and divergence is not None and subduction is not None:
        ...
        boundary = np.zeros((*spatial_shape, 3), dtype=np.float32)
        ...
        Image.fromarray(_cube_net_rgb(boundary_img), mode="RGB").save(boundary_path)
        results.append(
            VisualizationResult(
    ...
    if hotspot is not None and hotspot.max() > 0:
        base = np.empty((*spatial_shape, 3), dtype=np.float32)
        ...
        Image.fromarray(_cube_net_rgb(hot_img), mode="RGB").save(hotspot_path)
        results.append(
...
def tectonics_stage(context, deps, config_mapping):
    ...
    if isinstance(context.topology, CubedSphereGrid):
        unsupported = {"time_steps", "time_step", "wrap_x", "wrap_y"}.intersection(config_mapping)
        if unsupported:
            names = ", ".join(sorted(unsupported))
            raise ValueError(f"cubed_sphere tectonics does not use: {names}")
        shape = context.topology.face_shape
        plate_handle = context.arena.allocate_array(
            "tectonics_plate_field",
            (*shape, SPHERICAL_PLATE_FIELD_COMPONENTS),
            dtype=np.float32,
        )
        scalar_handles = {
            "convergence": context.arena.allocate_array(
                "tectonics_convergence", shape, dtype=np.float32
            ),
            "divergence": context.arena.allocate_array(
                "tectonics_divergence", shape, dtype=np.float32
            ),
            "shear": context.arena.allocate_array("tectonics_shear", shape, dtype=np.float32),
            "subduction": context.arena.allocate_array(
                "tectonics_subduction", shape, dtype=np.float32
            ),
            "hotspot": context.arena.allocate_array("tectonics_hotspot", shape, dtype=np.float32),
        }
        plate_view = plate_handle.mutable_view()
        scalar_views = {name: handle.mutable_view() for name, handle in scalar_handles.items()}
        with context.timed("tectonics_kernel"):
            metadata = run_cubed_sphere_tectonics(
                ...
                convergence_field=scalar_views["convergence"],
                divergence_field=scalar_views["divergence"],
                shear_field=scalar_views["shear"],
                subduction_field=scalar_views["subduction"],
                hotspot_field=scalar_views["hotspot"],
            )
        convergence_handle = scalar_handles["convergence"]
        divergence_handle = scalar_handles["divergence"]
        shear_handle = scalar_handles["shear"]
        subduction_handle = scalar_handles["subduction"]
        hotspot_handle = scalar_handles["hotspot"]
        convergence_view = scalar_views["convergence"]
        divergence_view = scalar_views["divergence"]
        ...
        shear_handle = context.arena.allocate_grid(
            "tectonics_shear", (height, width), dtype=np.float32
        )
        ...
        metadata["kinematic_model"] = "provisional_rectangular_v4"
        metadata["velocity_basis"] = "grid_uv"

    if isinstance(context.topology, CubedSphereGrid):
        continental_fraction_actual = float(
            np.sum(plate_view[..., 1] * context.topology.cell_areas)
            / np.sum(context.topology.cell_areas)
        )
    else:
    ...
    metadata.update(
        {
            ...
            "hotspot_density": config.hotspot_density,
            "subduction_bias": config.subduction_bias,
            "plate_field_components": int(plate_view.shape[-1]),
        }
    )
