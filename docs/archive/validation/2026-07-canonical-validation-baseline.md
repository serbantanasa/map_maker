...
## Selected-Basin Prospective Fluvial Profile

The sparse constraint pass builds 966 candidate bed nodes and 960 physical
edges in six components. The components are separated where zero-width
connectors cross process-excluded depression support. All 982 physical
centerline memberships receive candidate profile records; connectors receive
none. No profile or sediment budget is applied to raster terrain.
...
| Maximum candidate conditioning incision | `536.01 m` |
| Potential incision diagnostic | `118.01 km3` |
| Prospective channel excavation | `36.92 km3` |
| Prospective floodplain deposition | `19.72 km3` |
| Inland terminal deposition | `0 km3` |
| Prospective ocean sediment export | `17.21 km3` |
| Sediment conservation residual | `0 m3` |
| Bank erosion | `0 m3` |
| Maximum applied full-cell mean change | `0 m` |
| Maximum applied coarse-parent mean change | `0 m` |
...
The prospective-to-potential ratio is approximately `0.313`, but it is not a
calibration target. The first value is the candidate channel prism required to
condition an unresolved terrain prior; the second is inherited from coarse
reach relief. Regional refinement must decide what terrain history can realize.
The maximum candidate depth remains a graph and waterbody diagnostic, not an
accepted valley cut.
...
The pre-incision transient state is not accepted standing water.
`surface_water_ready_for_soils` remains false until bounded outlet correction
either converges or explicitly hands round-limited spill geometry to regional
refinement. The current class and area distribution remains provisional.
...
| Metric | Canonical result |
| --- | ---: |
| Requested candidates | `1,249` |
| Applied / bounded-accepted | `1,149 / 100` |
| Corrected cells | `3,008` |
| Corrected active area | `68,315 km2` (`1.930%`) |
| Subgrid eroded volume | `9.803 km3` |
| Maximum bed incision | `250.05 m` |
| Maximum cell-mean lowering | `4.31 m` |
| Receiver-change area | `130,027 km2` (`3.674%`) |
| Cycle repair | `7` rounds, `8` ordinary cells |
| Physical trunk anchors preserved | `966 / 966` |

Monthly balance and correction genuinely converge in 12 rounds with no regional
deferral. Across all rounds, 3,005 requests produce 1,884 applied corrections
and about `14.502 km3` of bounded outlet erosion. The final graph publishes
`surface_water_ready_for_soils = 1`; annual candidate-network water balance
closes to `5.2e-16` relative error.

Final accepted standing-water mean area is `167,211 km2`, or `4.72%` of the
selected basin's active area. Of 4,684 candidates, 4,672 are permanent lakes,
12 are hydrologic wetlands, and none are seasonal. The global lake-land fraction
is also above the current Earth diagnostic. Multi-seed and multi-resolution
comparison must improve count, size distribution, hydroperiod, and bathymetry
before these labels are treated as Earth-like.
...
which is inside the band. See the archived
[Earth biosphere validation profile](../completed-stage-specs/biosphere/09d_earth_biosphere_validation.md)
for sources, climate strata, tolerances, and pass semantics.
...
## Physical Atlas V5 River-Continuity Candidate

The 2026-07-21 rerun separates physical channels from lake/spill connectors,
conditions sparse river profiles against registered water surfaces, and extends
those surfaces only over submerged child-cell approaches. All six face-64
worlds pass `validate-biosphere` after the correction.

| Metric | Six-seed range | Canonical face-128 seed 42 |
| --- | ---: | ---: |
| Longest source-to-terminal hydrologic path | `4,335-6,600 km` | `4,490 km` |
| Confirmed physical channel on longest path | `2,767-3,935 km` | `3,170 km` |
| Physical-channel fraction of longest path | `57.9-82.2%` | `60.4%` |
| Lake area / land | `1.72-2.92%` | `2.37%` |
| Lake volume | `135,597-236,054 km3` | `190,072 km3` |
| Maximum prospective channel incision | `376-789 m` | `200 m` |

The canonical Equal Earth output is `4096x1994`, renders `563` connected river
reaches, and has PNG SHA-256
`46144f8dbcb07d84821254762c460ce2f461a5c16409bfde086e902140dd0779`.
Its atlas status is complete, but milestone acceptance remains pending explicit
human disposition of the projected map.
